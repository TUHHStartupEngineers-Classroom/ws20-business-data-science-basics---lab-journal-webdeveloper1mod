...
bike_orderlines_joined_tbl$category       
 ...
 bike_orderlines_joined_tbl %>% 
  select(category) %>%
  filter(str_detect(category, "Enduro")) %>% 
    unique()
  ...
  #writting Files
   ...
   bike_orderlines_wrangled_tbl %>% 
     ...
     set_names(names(.) %>% str_replace_all("\\.", "_"))
       ...
       sales_by_loc_tbl  <- bike_loc_wrangled_tbl  %>% 
          select(state, city, order_date, total_price, quantity) %>% 
            arrange(desc(quantity)) 
            
                       
                       
        # Ploting the data
          {r plot, fig.width=1000, fig.height=700}
      sales_by_loc_tbl %>% 
        ggplot(aes(x = city, y = state)) + 
